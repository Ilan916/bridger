...
function getFiberFromNode(node: Element | null): any {
  if (!node) return null
...
function walkFiber(fiber: any, components: Set<string>, depth = 0): void {
  if (!fiber || depth > 50) return // sécurité anti boucle infinie
  ...
  if (fiber.sibling) walkFiber(fiber.sibling, components, depth + 1)
}

// API publique — retourne les noms de composants sur la page courante
export function scanCurrentPage(): string[] {
  try {
